...
write.table( df_eachvideo, file = "FILENAME", sep = ",", col.names = FALSE, ) #Filename in the form "Video number_ participant ID.csv" ex 1_001.csv

df_eachvideo <- read.csv("2 .csv", header=FALSE)
value <- df_eachvideo$V6[1]
df_eachvideo$V6 <- df_eachvideo$V6 - value
df_eachvideo$V1 <- NULL
write.table( df_eachvideo, file = "FILENAME", sep = ",", col.names = FALSE, )
